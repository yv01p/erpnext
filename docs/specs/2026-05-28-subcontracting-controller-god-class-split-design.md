...
**Note:** Three of these stays-on-controller methods have bodies that call `__`-prefixed methods being extracted. Their bodies require rewriting per §3.4's translation rule — see §3.6 below.

### 3.6 — Stays-on-controller methods that nonetheless need body rewrites

Three methods listed in §3.5 stay on the controller but call `__`-prefixed methods that *move* to helpers. After extraction, the mangled lookup `self._SubcontractingController__foo` (from Python's name-mangling of `self.__foo()` inside the class) finds no attribute → `AttributeError`. The bodies of these three methods must therefore be rewritten in the commits that extract their callees, per §3.4's translation rule (which applies regardless of whether the *caller* method moves).

| Method (stays) | Line | Calls (moves) | Required rewrite | Touched in |
|---|---|---|---|---|
| `set_materials_for_subcontracted_items` | L1109 | `self.__identify_change_in_item_table()` | `data_assembly._identify_change_in_item_table(self)` | T2 |
| `set_materials_for_subcontracted_items` | L1110 | `self.__prepare_supplied_or_received_items()` | `self.supplied_items_helper._prepare_supplied_or_received_items()` | T3 |
| `set_materials_for_subcontracted_items` | L1111 | `self.__validate_supplied_or_received_items()` | `self.supplied_items_helper._validate_supplied_or_received_items()` | T3 |
| `set_consumed_qty_in_subcontract_order` | L1143 | `self.__get_subcontract_orders()` | `data_assembly._get_subcontract_orders(self)` | T2 |
| `set_consumed_qty_in_subcontract_order` | L1151 | `self.__update_consumed_materials(doctype, return_consumed_items=True)` | `data_assembly._update_consumed_materials(self, doctype, return_consumed_items=True)` | T2 |
| `set_subcontracting_order_status` | L1275 | `self.__get_subcontract_orders` *(no parens — pre-existing dead-line)* | **Delete the line.** Currently a no-op method-reference; after T2 it becomes a hard `AttributeError`. `self.subcontract_orders` is populated by correct calls elsewhere (e.g., L1055, L1143) so deletion preserves observable behavior. | T2 |

`set_consumed_qty_in_subcontract_order` L1159 (`self.__update_consumed_qty_in_subcontract_order(itemwise_consumed_qty)`) is safe — `__update_consumed_qty_in_subcontract_order` also stays on the controller; both ends share the class scope, so mangling resolves correctly.
