...
class ImportFile:
	...
	def _read_raw(self):
		...
		if ext == "csv":
			...
			frappe.throw(_("Unsupported file type: .{0}. Upload a CSV or XLSX.").format(ext))
...
def _build_errored_xlsx(all_rows, party_type):
	...
	fieldname_row = [cstr(v) for v in (all_rows[2] if len(all_rows) > 2 else [])]
	...
	FIELDNAME_FILL = PatternFill("solid", fgColor="F2F2F2")
	DATA_FILL = PatternFill("solid", fgColor="FFFFFF")
	...
	for c_idx, (section, label, fieldname) in enumerate(
		zip(section_row, label_row, fieldname_row, strict=False), start=1
	):
		is_err = c_idx == n_cols and fieldname == "_error_message"
		dark = SECTION_DARK.get(section, FALLBACK_DARK)
		light = SECTION_LIGHT.get(section, FALLBACK_LIGHT)
		...
		c3 = ws.cell(row=3, column=c_idx, value=fieldname)
		c3.fill = ERROR_LIGHT if is_err else FIELDNAME_FILL
		c3.font = Font(italic=True, size=8, color="9C0006" if is_err else "595959")
