...
class TestPartyImport(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
...
def _delete_test_data():
	for doctype, names in [
		("Customer", [CUSTOMER_CODE, CUSTOMER_NAME, "TC-IMPORT-002", "Test Customer 2"]),
		...
		for name in names:
			...
			for addr in frappe.get_all(
				...
				frappe.delete_doc("Address", addr, force=True, ignore_missing=True)
			for cont in frappe.get_all(
				"Dynamic Link",
				filters={"link_doctype": doctype, "link_name": name, "parenttype": "Contact"},
				pluck="parent",
			):
				frappe.delete_doc("Contact", cont, force=True, ignore_missing=True)
